...
class FlutterLitertLmPlugin : FlutterPlugin, MethodCallHandler, EventChannel.StreamHandler {
    ...
    override fun onMethodCall(call: MethodCall, result: Result) {
        when (call.method) {
            ...
            "hasSpeculativeDecodingSupport" -> handleHasSpeculativeDecodingSupport(call, result)
            "getBenchmarkInfo" -> handleGetBenchmarkInfo(call, result)
    ...
    private fun handleCreateEngine(call: MethodCall, result: Result) {
        scope.launch {
            try {
                ...
                val cpuThreads = call.argument<Int>("cpuThreads")
                val maxNumTokens = call.argument<Int>("maxNumTokens")
                val maxNumImages = call.argument<Int>("maxNumImages")
                val requestedSpeculativeDecoding =
                    call.argument<Boolean>("enableSpeculativeDecoding")
                val enableBenchmark = call.argument<Boolean>("enableBenchmark") ?: false

                val backend = parseBackend(backendName, cpuThreads)
                val visionBackend = visionBackendName?.let { parseBackend(it, cpuThreads) }
                val audioBackend = audioBackendName?.let { parseBackend(it, cpuThreads) }
                ExperimentalFlags.enableBenchmark = enableBenchmark
                ExperimentalFlags.enableSpeculativeDecoding =
                    resolveSpeculativeDecoding(modelPath, requestedSpeculativeDecoding)
    ...
    private fun handleHasSpeculativeDecodingSupport(call: MethodCall, result: Result) {
        scope.launch {
            try {
                val modelPath = call.argument<String>("modelPath")!!
                val supported = hasSpeculativeDecodingSupport(modelPath)
                withContext(Dispatchers.Main) {
                    result.success(supported)
                }
            } catch (e: Exception) {
                withContext(Dispatchers.Main) {
                    result.error("CAPABILITIES_ERROR", e.message, e.stackTraceToString())
                }
            }
        }
    }

    private fun handleGetBenchmarkInfo(call: MethodCall, result: Result) {
        scope.launch {
            try {
                val convId = call.argument<String>("conversationId")!!
                val conversation = conversations[convId]
                ...
                withContext(Dispatchers.Main) {
                    result.success(benchmarkInfoToMap(info))
                }
            } catch (e: Exception) {
                withContext(Dispatchers.Main) {
                    result.error("BENCHMARK_ERROR", e.message, e.stackTraceToString())
                }
            }
        }
    }
    ...
    private fun parseBackend(name: String, cpuThreads: Int? = null): Backend = when (name) {
        "gpu" -> Backend.GPU()
        "npu" -> Backend.NPU(context.applicationInfo.nativeLibraryDir)
        else -> Backend.CPU(cpuThreads)
    }

    private fun resolveSpeculativeDecoding(modelPath: String, requested: Boolean?): Boolean {
        if (requested == false) return false
        return hasSpeculativeDecodingSupport(modelPath)
    }

    private fun hasSpeculativeDecodingSupport(modelPath: String): Boolean {
        return try {
    ...
    private fun benchmarkInfoToMap(info: BenchmarkInfo): Map<String, Any> = mapOf(
        "initTimeInSecond" to info.initTimeInSecond,
        "timeToFirstTokenInSecond" to info.timeToFirstTokenInSecond,
        "lastPrefillTokenCount" to info.lastPrefillTokenCount,
        "lastDecodeTokenCount" to info.lastDecodeTokenCount,
        "lastPrefillTokensPerSecond" to info.lastPrefillTokensPerSecond,
        "lastDecodeTokensPerSecond" to info.lastDecodeTokensPerSecond,
    )
