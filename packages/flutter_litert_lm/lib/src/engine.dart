...
class LiteLmEngine {
  ...
  static Future<LiteLmEngine> create(LiteLmEngineConfig config) async {
    final id = await FlutterLitertLmPlatform.instance.createEngine(
      config.toMap(),
    );
    return LiteLmEngine._(id);
  }

  /// Returns whether the model file advertises native MTP speculative decoding.
  static Future<bool> hasSpeculativeDecodingSupport(String modelPath) {
    return FlutterLitertLmPlatform.instance.hasSpeculativeDecodingSupport(
      modelPath,
    );
  }
