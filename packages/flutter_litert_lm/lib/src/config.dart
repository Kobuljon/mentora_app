...
class LiteLmEngineConfig {
  ...
  /// Number of CPU threads to use when [backend] falls back to CPU.
  ///
  /// Ignored by GPU and NPU backends.
  final int? cpuThreads;

  /// Maximum token budget requested from the native runtime.
  final int? maxNumTokens;

  /// Maximum number of images accepted by the native runtime.
  final int? maxNumImages;

  /// Whether to enable Gemma 4 MTP speculative decoding.
  ///
  /// When null, the Android runtime auto-detects model support and enables it
  /// only for compatible model files.
  final bool? enableSpeculativeDecoding;

  /// Enables native LiteRT-LM benchmark counters when supported.
  ...
  Map<String, dynamic> toMap() => {
    ...
    if (cpuThreads != null) 'cpuThreads': cpuThreads,
