...
class MethodChannelFlutterLitertLm extends FlutterLitertLmPlatform {
  ...
  Future<void> disposeConversation(String conversationId) async {
    await _methodChannel.invokeMethod('disposeConversation', {
  ...
  Future<Map<String, dynamic>> sendMessage(
    ...
    final result = await _methodChannel.invokeMethod<Map>('sendMessage', {
      'conversationId': conversationId,
      'contents': contents,
  ...
  Stream<Map<String, dynamic>> sendMessageStream(
    ...
    return _eventChannel
        .receiveBroadcastStream({'conversationId': conversationId})
        .map((event) {
          if (event is Map) {
  ...
  Future<int> countTokens(String engineId, String text) async {
    final result = await _methodChannel.invokeMethod<int>('countTokens', {
      'engineId': engineId,
      'text': text,
    });
    return result!;
  ...
  @override
  Future<bool> hasSpeculativeDecodingSupport(String modelPath) async {
  ...
  Future<Map<String, dynamic>> getBenchmarkInfo(String conversationId) async {
    final result = await _methodChannel.invokeMethod<Map>('getBenchmarkInfo', {
      'conversationId': conversationId,
    });
